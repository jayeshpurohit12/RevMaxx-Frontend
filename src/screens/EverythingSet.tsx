...
export default function EverythingIsSet({navigation}: {navigation: any}) {
  ...
  const checkPermissions = async () => {
    ...
    const camStatus = await PermissionsAndroid.check(
  ...
  return (
    <>
      <Image source={require('../assets/images/imgs/EverythingSetImg.png')} />
      <View
        style={{
          width: '100%',
          gap: 24,
        }}>
        <Text style={styles.titleText}>Everything is set!</Text>
        <View
          style={{
            gap: 12,
          }}>
          {['Microphone Permissions', 'Camera Permissions'].map(
            (item, index) => (
                    ...
                    <Image
                      source={
                        item === 'Microphone Permissions'
                          ? require('../assets/images/icons/microphone.png')
